...
import { logout, isLoggedIn, currentEmail, ensureSessionLoaded } from '../cloudAccount.js';
...
function syncStatus() {
  ...
  return { label: 'Ažurno', small: 'Svi podaci su sačuvani na nalogu' };
}
...
export async function renderPodesavanja(root, go) {
  ...
  const status = loggedIn ? syncStatus() : null;
  ...
  root.innerHTML = `
    ...
    <div class="card">
      ...
      ${loggedIn ? `
        ...
        <div class="settings-row">
          <div class="l">${esc(status.label)}<small>${esc(status.small)}</small></div>
      ...
      ` : '<div class="empty-hint">Nisi ulogovana</div>'}
    </div>

    <div class="card">
      <h2>Podaci</h2>
      <div class="hint">Podaci se čuvaju na nalogu. Brisanje je trajno i ne može se opozvati.</div>
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async () => {
      if (!confirm('Izlogovati se? Sledeći put ćeš morati ponovo da se uloguješ.')) return;
      await logout();
      toast('Izlogovana');
